...
public class PetAttack : FixedMonoBehaviour
{
    ...
    private void OnAttack()
    {
        ...
        if (_timer > 0) return;
        GameObject clone = PoolManager.Instance.SpawnFromPool(_bulletPrefab.tag, _pet.position, Quaternion.identity);
        Vector3 direction = (_nearestTarget.position - _pet.position).normalized;
        clone.GetComponent<Bullet>().ChangeDirection(direction);
        clone.GetComponentInChildren<BulletSensor>().SetAttack(Attack);
